---
title: Appendix 2 - Validation of the microclimate model
...
knitr::opts_knit$set(root.dir = '/Users/usuario/Dropbox/1_papers/CAS/I_alpestris/0_living_underground/ThermHydroBehav/data/micro_studenec')
```

In this document we run NicheMapR's microclimate model at the Institute of Vertebrate Biology, Studenec (Czechia) using NCEP data as forcing variables. We compare model results with logged data for the period XXX to XXX. 

Humidity and temperature dataloggers were placed outdoors of the institute at different depths. 

**Note:**: If you want to reproduce this script make sure you are in the same directory as the weather and data logger data.
...
```{r}
soil <- data.frame(micro$soil) # explain each output
shadsoil <- data.frame(micro$shadsoil)
humid <- data.frame(micro$humid)
shadhumid <- data.frame(micro$shadhumid)
...
par(mfrow=c(2,1), mar=c(2.5, 4, 0.5, 0.1))
with(metout, plot(micro$dates, SNOWFALL, type='l', xlab='', ylab='Snowfall (cm)'))
with(metout, plot(micro$dates, SNOWDEP, type='l', xlab='', ylab='Snow depth (cm)'))
```
